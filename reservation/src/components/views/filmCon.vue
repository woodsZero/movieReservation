<template>
	<section class="pr v-slide-content">
		<ul class="v-slide-box pa" ref='vSlide' :class="{'slide-action': true}">
			<li v-for="(item,index) in lists" :key="index" @click="getId(item.mId)">
...
export default{
	...
	mounted(){
		// console.log(this.$store.state.selFilm,'初始')
		// this.$store.dispatch()
	},
	methods: {
		getId(mId){
			...
			this.selFilm = this.getDataById(this.lists,mId)
			let selFilm = this.selFilm
